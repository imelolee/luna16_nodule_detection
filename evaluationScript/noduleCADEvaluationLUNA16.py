...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    CPMOutputFile = open(os.path.join(outputDir,'CPMAnalysis.txt'),'w')
    ...
    CPMOutputFile.write('FROC at points: {}\n'.format(thres))
    
    for th in thres:
        if max(fps-th)<0:
            find_nearest_idx = -1
        else:
            find_nearest_idx = find_nearest(fps, th)
        CPMOutputFile.write('fps: {}, sensitivity: {}\n'.format(th, sens[find_nearest_idx]))
        sen.append(sens[find_nearest_idx])
    CPMOutputFile.write('=============================================\naverage FROC: {}'.format(np.mean(sen)))
